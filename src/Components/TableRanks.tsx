...
function TableRanks({ data, selectRankToCompare }: Props) {
    ...
    return (
        <div className="d-flex justify-content-between me-3">
            {ranks.map(r => (
                <OverlayTrigger key={r.key} overlay={<Tooltip>{"compare"}</Tooltip>}>
                    <span className="d-inline-block">
                        <Button onClick={() => selectRankToCompare(r.key)} variant={r.value === min ? "primary" : "secondary"} className="align-items-center font-monospace" size="sm">
                            <div className="d-flex align-items-center">
                            Rank_{r.key}
                                <Badge bg="dark" className="ms-2">{r.value}%</Badge>
                            </div>
                        </Button>
                    </span>
                </OverlayTrigger>
